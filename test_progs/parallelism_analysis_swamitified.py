...
for runcount in range(5,21):
    ...
    print(benchmark + " problem size is " + str(test_q_cnt) + " qubits running 1 - " + str(max_copies) + " circuits in parallel.")
    print(benchmark + " problem size is " + str(test_q_cnt) + " qubits running 1 - " + str(max_copies) + " circuits in parallel.", file=f)
    ...
    for connectivity_map in connectivity_maps:
        ...
        collect_benchmark_data_analytical(
                                                ...
                                                result=results, 
                                                result_2=results_single                                       #The return array to which results are appended
                                                )
        ...
        runtime_results = []
        for run in results_single:
            runtime_no_copies = results_single[0][7] * 1024
    ...
    plt.ylabel("Speedup")
    plt.title(pltname)
    all_copy_values = sorted(set(row[0] for arch in grouped_runtime_results for row in arch))
    plt.xticks(all_copy_values)

    # Optional: explicitly set x-axis limits to avoid crowding or cutoff
    plt.xlim(min(all_copy_values), max(all_copy_values))
    plt.grid(True)
    plt.legend()
    ax = plt.gca()
